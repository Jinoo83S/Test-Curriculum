...
globalThis.HIS_AUTOASSIGN_BUILD = "2026-06-24-r115-sidebar-syntax-hotfix-r117";
...
export function createAutoAssignAll(deps) {
  ...
  function compactAutoAssignSnapshotMeta(meta = {}) {
    ...
    return {
      ...
      validatorVersion: String(meta.validatorVersion || "2026-06-24-r115-sidebar-syntax-hotfix-r117"),
  ...
  function attachCanonicalMetaToMatchingSnapshots(domain, canonicalMeta = {}, canonicalEntries = []) {
    ...
    const compact = compactAutoAssignSnapshotMeta({
      ...canonicalMeta,
      schemaVersion: canonicalMeta.schemaVersion || "2026-06-24-r115-sidebar-syntax-hotfix-r117",
    ...
    for (const targetSlot of targetSlots) {
      ...
      if (checkPlacementValid(item, targetSlot, placed, checkOptions)) {
        ...
        const fixed = normalizeTimetableEntry({
          ...
          autoEngine: "fresh-csp-r117-coverage-onemove-direct",
      ...
      for (const block of blockers) {
        ...
        for (const moveSlot of moveSlots) {
          ...
          const fixed = normalizeTimetableEntry({
            ...entry,
            autoBlockKey: probeKey,
            autoEngine: "fresh-csp-r117-coverage-onemove",
            autoCoverageRepair: true
          });
  ...
  function freshTryPlaceSingleCoverageCard(cardId, placed = [], baseSlots = [], options = {}) {
    ...
    if (direct.ok) {
      ...
      added.forEach(e => { e.autoEngine = "fresh-csp-r117-coverage-direct"; });
    ...
    const fixed = normalizeTimetableEntry({
      ...
      autoEngine: "fresh-csp-r117-coverage-fill",
      autoCoverageRepair: true,
      forced: true
    });
    placed.push(fixed);
  ...
  async function runFreshAutoPlacementEngine(context = {}) {
    ...
    return {
      ...
      stats: {
        engine: "fresh-csp-groupcard-r117",
  ...
  async function autoAssignAll() {
    ...
    try {
      await updateProgress({
        ...
        failed: 0,
        currentCard: "-",
        log: `대상 학년: ${formatAutoActiveGrades(activeGrades)} · 엔진: fresh-csp-groupcard-r117`
      }, true);
