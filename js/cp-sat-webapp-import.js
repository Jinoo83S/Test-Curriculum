...
// r178: 현재 웹앱 연결 + solver 전송 JSON에서 학생 객체/시간표 학생필드 제거 + 결과 적용 시 교실 배정 보존.
// ================================================================
...
function stripSolverOnlyState(state) {
  ...
  // r178 원칙: 시간표/solver 전송 JSON에는 학급 학생 객체를 싣지 않습니다.
...
function makeSolverState(appState) {
  ...
  const wrapped = {
    ...
    exportedAt: nowIso(),
    source: "HIS webapp r178 CP-SAT API bridge",
    data: deepClone(data),
  };
...
export function setupCpSatWebappImport(ctx = {}) {
  ...
  function normalizeSolvedEntry(rawEntry) {
    ...
    // r178 핵심: 현재 운영 웹앱의 state.js가 오래된 경우 normalizeTimetableEntry가
