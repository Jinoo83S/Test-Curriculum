...
function openPlacedTemplateEditor(templateId, ctx = {}) {
  ...
  chk.type = "checkbox";
  chk.checked = draft.useSemesterOverrides;
  chk.disabled = !canEdit();
  const chkLabel = document.createElement("span");
  chkLabel.textContent = "1·2학기 이름/교사 분리";
  ...
  const renderSemGrid = () => {
    ...
    semGrid.querySelectorAll("input,select").forEach(el => {
      el.disabled = !canEdit() || !draft.useSemesterOverrides;
    });
  };
...
function createMergedPlacedCard(templateId, grade, rowData) {
  ...
  const meta = buildExpandedMeta(item, item); card.append(top, meta); attachExpandClick(card, meta, templateId, grade, rowData, "merged"); return card;
}
