...
// r391: aSc 교사 미팅을 학생·학급·교실과 분리하고 해당 교사의 수업 배정만 차단합니다.
// ================================================================

const DAY_LABELS = Object.freeze(["월", "화", "수", "목", "금"]);
const STYLE_ID = "ttTeacherEventsStyleR391";
...
export function normalizeTeacherEvent(raw = {}, options = {}) {
  ...
  return {
    ...
    sourceSheet: clean(raw.sourceSheet),
    sourceKind: clean(raw.sourceKind),
    sourceRows: asArray(raw.sourceRows).map(Number).filter(Number.isFinite),
...
export function createTeacherEventsManager(context = {}) {
  ...
  const persist = async label => {
    if (typeof persistTimetable === "function") {
      await persistTimetable();
    } else {
      scheduleSave("timetable", { immediate: true });
    }
    recomputeConflicts();
    renderAll();
    try { console.info(`[teacher-events:r391] ${label} count=${events().length}`); } catch (_) {}
  };
...
일정을 저장하면 해당 수업은 충돌로 표시되며 다른 시간으로 옮겨야 합니다. 계속할까요?`)) return;
        ...
        selectedId = next.id;
        try {
          await persist(item ? "updated" : "created");
          render();
        } catch (error) {
          domain.teacherEvents = previousEvents;
          try { recomputeConflicts(); } catch (_) {}
          try { renderAll(); } catch (_) {}
          render();
          console.error("[teacher-events:r391] save failed", error);
          alert(`교사 일정 저장에 실패했습니다. 변경 전 상태로 되돌렸습니다.\n\n${error?.message || error}`);
        }
      });
      formEl.querySelector('[data-action="delete"]')?.addEventListener("click", async () => {
        ...
        try {
          ...
          try { renderAll(); } catch (_) {}
          render();
          console.error("[teacher-events:r391] delete failed", error);
          alert(`교사 일정 삭제 저장에 실패했습니다. 삭제 전 상태로 되돌렸습니다.\n\n${error?.message || error}`);
        }
      });
    };
