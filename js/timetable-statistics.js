...
function teacherDetail(row) {
  ...
  return `<h3>${escapeHtml(row.name)} · 전체 ${row.total}시수</h3><div class="tt-stat-detail-grid">${days}</div>`;
}
...
export function openStatisticsDialog(context = {}) {
  ...
  const rebuild = () => {
    teacherReport = buildTeacherStatistics({
      ...
      teachers: context.getTeachers?.() || context.teachers || [], teacherConstraints: context.getTeacherConstraints?.() || context.teacherConstraints || {},
      ...
      periodCount: context.getPeriodCount?.() || context.periodCount || 7,
    });
  ...
  const render = () => {
    ...
    if (activeTab === "teacher") {
      ...
      summary.innerHTML = [[teacherReport.teacherCount,"표시 교사"],[teacherReport.referencedTeacherCount,"수업 배정 교사"],[teacherReport.totalTeacherPeriods,"전체 교사 점유시수"],[teacherReport.hardViolationTeacherCount,"강제 제한 초과"],[teacherReport.softViolationTeacherCount,"유연 제한 초과"]].map(([v,l])=>`<div class="tt-stat-card"><b>${v}</b><span>${l}</span></div>`).join("");
      head.innerHTML = `<tr><th>교사</th><th>전체</th>${DAY_LABELS.map(label=>`<th>${label}<br><small>시수/연속</small></th>`).join("")}<th>일일최대</th><th>최대연속</th><th>상태</th></tr>`;
      body.innerHTML = rows.length?rows.map(row=>`<tr class="tt-stat-row${selectedTeacher===row.key?" selected":""}" data-key="${escapeHtml(row.key)}"><td><b>${escapeHtml(row.name)}</b></td><td>${row.total}</td>${row.daily.map(day=>`<td>${day.count} / ${day.maxConsecutive}</td>`).join("")}<td>${row.maxPerDay}</td><td>${row.maxConsecutive}</td><td>${row.status==="hard"?"강제 초과":row.status==="soft"?"유연 초과":"정상"}</td></tr>`).join(""):`<tr><td class="tt-stat-empty" colspan="11">교사가 없습니다.</td></tr>`;
      detail.innerHTML = teacherDetail(teacherReport.rows.find(row=>row.key===selectedTeacher)); note.textContent = "교사ID + 요일 + 교시 기준 중복 제거";
    } else {
      const previousSort = sort.value || "name";
