...
import { appState, subscribeDomains, unsubscribeAll, setOnUpdate, scheduleSave, saveNow,
         ...
         exportLocalSnapshot, importLocalSnapshot, resetLocalSnapshot, exportFirestoreDiagnosticSnapshot } from "./state.js?v=2026-07-06-state-cache-unified-r232";
import { LOCAL_DEV_MODE } from "./local-dev.js?v=2026-07-06-state-cache-unified-r232";
import { versioned } from "./version.js?v=2026-07-06-state-cache-unified-r232";
import { openFirestoreUsageDialog } from "./firestore-usage.js?v=2026-07-06-state-cache-unified-r232";
import { openAppHealthCheckDialog } from "./app-health-check.js?v=2026-07-06-state-cache-unified-r232";
import { getTemplateById, getTemplateCardTitle, splitTeacherNames } from "./templates.js?v=2026-07-06-state-cache-unified-r232";
import { uid, clean, makeBtn, sectionLabel, gradeDisplay, escapeHtml, isProtectedWholeGradeLabel } from "./utils.js";
import { getRooms, getRoomById, renderRoomsView, updateRoom, formatHomeRoomClassLabel } from "./rooms.js?v=2026-07-06-state-cache-unified-r232";
import {
...
import { openTimetableExportDialog } from "./timetable-export.js?v=2026-07-06-state-cache-unified-r232";
...
function buildCurrentEntriesAuditSummary() {
  ...
  return {
    version: "r232-current-entries-audit",
    ok,
    summary,
...
async function ensureTimetableDataSyncedForOperation(reason = "") {
  ...
  if (persistentChanged && canEdit()) {
    try {
      ...
      console.warn(`[data-sync:r232] ${reason || "operation"} 전 데이터 정규화 저장 실패`, e);
      throw e;
    }
  }
  try {
    const msg = `[data-sync:r232] ${reason || "operation"} 전 정규화: teacher=${result.teacherChanged}, manual=${result.manualChanged || 0}, room=${result.roomChanged}, condition=${result.conditionChanged || 0}, meta=${result.metaChanged}, audit=${result.auditChanged || 0}, saved=${persistentChanged ? "yes" : "no"}`;
...
function buildScheduleConditionRuntimeSummary() {
  ...
  return {
    mode: "schedule-condition-runtime-r232",
  ...
  ensureTeacherCardsBottomTab();
  {
    ...
    if (sync.changed) {
      ...
      if (persistentChanged) {
        try { console.debug(`[data-sync:r232] 렌더 전 정규화(저장대기 없음) teacher=${sync.teacherChanged}, manual=${sync.manualChanged || 0}, room=${sync.roomChanged}, condition=${sync.conditionChanged || 0}, meta=${sync.metaChanged}, audit=${sync.auditChanged || 0}`); } catch (_) {}
      }
    }
  }
  recomputeConflicts();
