...
import { appState, subscribeDomains, unsubscribeAll, setOnUpdate, scheduleSave, saveNow,
         ...
         verifyPersistedTimetableState } from "./state.js?v=1.0.0-20260724.2";
...
const [
  ...
  import("./timetable-autoassign.js?v=1.0.0-20260724.2"),
  import("./timetable-grid.js?v=1.0.0-20260724.2"),
  ...
  import("./timetable-statistics.js?v=1.0.0-20260724.2"),
  import(versioned("./timetable-display-density.js")),
  import("./timetable-teacher-events.js?v=1.0.0-20260724.2"),
  import("./cp-sat-webapp-import.js?v=1.0.0-20260724.2").catch(err => {
    console.warn("[CP-SAT import] optional module load failed", err);
    return {};
  }),
]);

globalThis.HIS_TIMETABLE_RUNTIME_RELEASE = "r387-13";
console.info("[HIS runtime:r387-13] card-move duplicate conflict recompute removed");
...
// r387-6: drag/drop path diagnostics and stale drag-state recovery.
let dragTraceSeq = 0;
...
function beginDragTrace(data, source = "unknown") {
  ...
  try { console.info(`[drag-path:r387-13] #${trace.id} dragstart source=${source} kind=${trace.kind}`); } catch (_) {}
  return trace;
}
...
function noteDragOverTrace(target = "grid") {
  ...
  try { console.info(`[drag-path:r387-13] #${trace.id} dragover target=${target}`); } catch (_) {}
}
...
function noteDropTrace(data, day = null, period = null, target = "grid") {
  ...
  try {
    ...
    console.info(`[drag-path:r387-13] #${trace.id} drop target=${target} kind=${dragTraceKind(data)}${slot}`);
  } catch (_) {}
  return trace;
}
...
function noteDragSaveScheduled(reason = "timetable") {
  ...
  try { console.info(`[drag-path:r387-13] #${trace.id} save-scheduled domain=${reason}`); } catch (_) {}
}
...
function clearAppDragState(reason = "clear", { delay = 0 } = {}) {
  const run = () => {
    ...
    if (trace) {
      ...
      try {
        console.info(`[drag-path:r387-13] #${trace.id} end reason=${reason} dragover=${trace.dragOverSeen?1:0} drop=${trace.dropSeen?1:0} save=${trace.saveScheduled?1:0} elapsed=${elapsed}ms`);
      } catch (_) {}
    }
...
function normalizeTeacherAliasName(name = "") {
  ...
  // 이전 배치 잔존값: "진로"는 교과군명으로도 쓰이지만 교사명 위치에서는 "진로교사"가 정식 교사입니다.
...
function buildCurrentEntriesAuditSummary() {
  ...
  return {
    version: "r343-current-entries-audit",
    ok,
    summary,
...
async function ensureTimetableDataSyncedForOperation(reason = "") {
  ...
  if (persistentChanged && canEdit()) {
    try {
      ...
      console.warn(`[data-sync:r343] ${reason || "operation"} 전 데이터 정규화 저장 실패`, e);
      throw e;
    }
  }
  try {
    const msg = `[data-sync:r343] ${reason || "operation"} 전 정규화: teacher=${result.teacherChanged}, manual=${result.manualChanged || 0}, room=${result.roomChanged}, condition=${result.conditionChanged || 0}, meta=${result.metaChanged}, audit=${result.auditChanged || 0}, saved=${persistentChanged ? "yes" : "no"}`;
...
const SCHEDULE_CONDITION_STORE_VERSION = "r229";
...
function openScheduleConditionPopup() {
  ...
  try {
    ...
    console.error("[schedule-condition-popup:r229] render failed", e);
    const body = $("ttScheduleConditionPopupBody");
...
function buildScheduleConditionRuntimeSummary() {
  ...
  return {
    mode: "schedule-condition-runtime-r343",
...
function addEntry(data, options = {}) {
  ...
  if (manualReplace) {
    ...
    if (plan.blocked) {
      try { console.info(`[manual-replace:r387-13] kind=add replaced=0 blocked=fixed target=${e.day}:${e.period}`); } catch (_) {}
  ...
  try {
    console.info(`[manual-replace:r387-13] kind=add replaced=${displacementIds.size} blocked=0 target=${e.day}:${e.period}`);
  } catch (_) {}
  captureTimetableUndo(displacementIds.size ? "수업 교체 추가" : "수업 추가");
  if (displacementIds.size) {
    entries().filter(item => displacementIds.has(item.id)).forEach(item => markCardMoveAffectedEntry(item));
    ttDomain().entries = entries().filter(item => !displacementIds.has(item.id));
  }
  markCardMoveAffectedEntry(e);
  entries().push(e);
  scheduleSave("timetable");
  noteDragSaveScheduled("timetable");
  return e;
...
function removeEntry(id) {
  if (!canEdit()) return;
  resetCardMoveRenderScope();
  const removing = entries().find(e => e.id === id);
  if (removing) markCardMoveAffectedEntry(removing);
...
function recomputeConflicts() {
  // r387-13: detectConflicts 내부의 카드쌍 비교에서 동일 entry resolver가 반복 호출됩니다.
...
const cardMoveAffectedSlots = new Set();
const cardMoveAffectedClassCells = new Set();
let cardMoveRequiresFullGrid = false;

function markCardMoveAffectedSlot(day, period) {
  const d = Number(day);
  const p = Number(period);
  if (!Number.isInteger(d) || !Number.isInteger(p)) return;
  cardMoveAffectedSlots.add(`${d}:${p}`);
}

function markCardMoveAffectedClassCell(day, period, gradeKey, sectionIdx) {
  const d = Number(day);
  const p = Number(period);
  const g = clean(gradeKey);
  const s = Number.parseInt(sectionIdx, 10);
  if (!Number.isInteger(d) || !Number.isInteger(p) || !g || !Number.isInteger(s)) return;
  cardMoveAffectedClassCells.add(`${d}:${p}:${g}:${s}`);
}

function markCardMoveAffectedEntry(entry, day = entry?.day, period = entry?.period) {
  if (!entry) return;
  markCardMoveAffectedSlot(day, period);
  const seen = new Set();
  try {
    const occupancy = audienceForPlacement(entry);
    [...(occupancy?.classKeys || new Set())].forEach(key => {
      const info = classInfoFromAudienceClassKey(key);
      if (!info.gradeKey || !Number.isInteger(info.sectionIdx)) return;
      const sig = `${info.gradeKey}:${info.sectionIdx}`;
      if (seen.has(sig)) return;
      seen.add(sig);
      markCardMoveAffectedClassCell(day, period, info.gradeKey, info.sectionIdx);
    });
  } catch (_) {}
  if (!seen.size) {
    const grades = entryGradeKeys(entry).length ? entryGradeKeys(entry) : [entry.gradeKey].filter(Boolean);
    grades.forEach(gradeKey => markCardMoveAffectedClassCell(day, period, gradeKey, entry.sectionIdx ?? 0));
  }
}

function resetCardMoveRenderScope() {
  cardMoveAffectedSlots.clear();
  cardMoveAffectedClassCells.clear();
  cardMoveRequiresFullGrid = false;
}

function markCardMoveComplex() {
  cardMoveRequiresFullGrid = true;
}

function entryMatchesPartialGridCell(entry, cell) {
  if (!entry || !cell) return false;
  if (Number(entry.day) !== Number(cell.dataset.day) || Number(entry.period) !== Number(cell.dataset.period)) return false;

  if (currentView === "grade") return entryHasGrade(entry, currentGrade);
  if (["class", "all"].includes(currentView)) {
    const gradeKey = cell.dataset.gradeKey || currentGrade || entry.gradeKey || "";
    const sectionIdx = Number.parseInt(cell.dataset.sectionIdx || "0", 10) || 0;
    return entryMatchesClass(entry, { gradeKey, sectionIdx });
  }
  return false;
}

function rebuildPartialGridCell(cell) {
  const slotEntries = entries().filter(entry => entryMatchesPartialGridCell(entry, cell));
  cell.replaceChildren();
  if (!slotEntries.length) {
    const placeholder = document.createElement("div");
    placeholder.className = "tt-cell-ph";
  ...
  if (currentView === "all") {
    ...
    holder.className = "tt-cell-card-grid";
    holder.style.height = "100%";
    holder.style.setProperty("--tt-auto-cols", String(slotEntries.length || 1));
    slotEntries.forEach(entry => holder.appendChild(buildEntryCard(entry, { compact: true })));
  ...
  slotEntries.forEach(entry => {
    ...
    card.style.cssText += ";flex-shrink:0;width:100%";
    cell.appendChild(card);
  });
}

function renderAffectedGridCells() {
  if (cardMoveRequiresFullGrid || !["grade", "class", "all"].includes(currentView)) return false;
  if (!cardMoveAffectedSlots.size || cardMoveAffectedSlots.size > 6) return false;

  const grid = ttGrid();
  if (!grid) return false;
  let cells = [...grid.querySelectorAll('.tt-cell[data-day][data-period]')]
    .filter(cell => cardMoveAffectedSlots.has(`${Number(cell.dataset.day)}:${Number(cell.dataset.period)}`));

  if (currentView === "all") {
    if (!cardMoveAffectedClassCells.size || cardMoveAffectedClassCells.size > 12) return false;
    cells = cells.filter(cell => cardMoveAffectedClassCells.has(
      `${Number(cell.dataset.day)}:${Number(cell.dataset.period)}:${clean(cell.dataset.gradeKey)}:${Number.parseInt(cell.dataset.sectionIdx || "0", 10) || 0}`
    ));
  }
  if (!cells.length) return false;

  cells.forEach(rebuildPartialGridCell);
  return true;
}
...
function moveEntry(entryId, day, period) {
  ...
  resetCardMoveRenderScope();
  linkedEntries.forEach(item => markCardMoveAffectedEntry(item));
  if (!linkedEntries.length || linkedEntries.some(item => item.pinned)) return false;
  if (linkedEntries.length !== 1) markCardMoveComplex();
  ...
  if (plan.blocked) {
    try { console.info(`[manual-replace:r387-13] kind=move replaced=0 blocked=fixed target=${day}:${period}`); } catch (_) {}
    const names = localUniqueStrings((plan.pinnedEntries || []).map(item => entryTitle(item))).slice(0, 5).join(", ");
  ...
  if (displacedIds.size) {
    entries().filter(item => displacedIds.has(item.id)).forEach(item => markCardMoveAffectedEntry(item));
    ttDomain().entries = entries().filter(item => !displacedIds.has(item.id));
  }

  linkedEntries.forEach((item, index) => markCardMoveAffectedEntry(item, targetPlacements[index]?.day, targetPlacements[index]?.period));
  ...
  try {
    console.info(`[manual-replace:r387-13] kind=move replaced=${displacedIds.size} blocked=0 target=${day}:${period} moved=${linkedEntries.length}`);
  } catch (_) {}
  scheduleSave("timetable");
  noteDragSaveScheduled("timetable");
  return true;
...
function renderVisibleBottomPanelAfterCardMove(label, startedAt) {
  ...
  cardMoveBottomRefreshTimer = setTimeout(() => {
    ...
    try {
      console.info(`[card-move-panel:r387-13] kind=${label} panel=${panel} elapsed=${Math.round(performance.now()-panelStartedAt)}ms total=${Math.round(performance.now()-startedAt)}ms`);
    } catch (_) {}
  }, 0);
}
...
function scheduleRuntimeMetaAfterCardMove(label, startedAt) {
  ...
  cardMoveRuntimeMetaTimer = setTimeout(() => {
    ...
    try {
      console.info(`[card-move-meta:r387-13] kind=${label} runtimeMeta=${Math.round(performance.now()-metaStartedAt)}ms total=${Math.round(performance.now()-startedAt)}ms`);
    } catch (_) {}
  }, 0);
}
...
function finishCardMoveRender(label, startedAt = performance.now()) {
  ...
  // r387-13: 부분 셀 재생성 후 화면 카드와 entries 상태가 어긋나는 사례가 확인되어
  // 데이터 정합성이 검증된 전체 격자 렌더로 임시 복귀한다.
  // 충돌 계산 캐시와 runtimeMeta 지연 최적화는 그대로 유지한다.
  renderGrid();
  const gridMs = Math.round(performance.now() - gridStartedAt);
  const gridMode = "full-integrity";

  const barStartedAt = performance.now();
  renderConflictBar();
  const conflictBarMs = Math.round(performance.now() - barStartedAt);
  ...
  try {
    console.info(`[card-move:r387-13] kind=${label} renderAll=0 conflictRecompute=1 conflicts=${conflictMs}ms runtimeMeta=deferred grid=${gridMs}ms gridMode=${gridMode} conflictBar=${conflictBarMs}ms immediate=${immediateMs}ms elapsed=${Math.round(performance.now()-startedAt)}ms entries=${entries().length}`);
  } catch (_) {}
}
...
function handleDrop(data, day, period) {
  ...
  if (data.kind === "group" && data.groupId) {
    resetCardMoveRenderScope(); markCardMoveComplex();
  ...
  ensureTeacherCardsBottomTab();
  {
    ...
    if (sync.changed) {
      ...
      if (persistentChanged) {
        try { console.debug(`[data-sync:r343] 렌더 전 정규화(저장대기 없음) teacher=${sync.teacherChanged}, manual=${sync.manualChanged || 0}, room=${sync.roomChanged}, condition=${sync.conditionChanged || 0}, meta=${sync.metaChanged}, audit=${sync.auditChanged || 0}`); } catch (_) {}
      }
    }
  }
  recomputeConflicts();
...
function buildDragPreviewReplacementPlan(candidates, excludeIds = []) {
  const candidateList = (Array.isArray(candidates) ? candidates : [candidates]).filter(Boolean);
  const movingIds = [...new Set((excludeIds || []).filter(Boolean))];
  const targetPlacements = candidateList.map(candidate => {
    const occupancy = manualReplacementOccupancy(candidate);
    return {
      entryId: candidate.id,
      day: candidate.day,
      period: candidate.period,
      classKeys: [...(occupancy.classKeys || new Set())],
      roomIds: effectiveRoomIdsForEntry(candidate),
    };
  });
  if (!targetPlacements.length) return { blocked: false, displacementIds: [] };
  return buildManualReplacementPlan({
    entries: entries(),
    movingEntryIds: movingIds,
    targetPlacements,
    getOccupancy: item => manualReplacementOccupancy(item),
    getOccurrenceEntries: item => manualOccurrenceEntries(item),
  });
...
function markDragPreviewCell(cell, data) {
  ...
  // r387-13: 실제 수동배치와 동일하게 교체 예정 카드는 충돌 미리보기에서 제외합니다.
  // 고정수업은 replacementPlan.blocked 상태로 유지하여 계속 빨간색으로 표시합니다.
  const replacementPlan = buildDragPreviewReplacementPlan(candidates, excludeIds);
  const previewExcludeIds = replacementPlan?.blocked
    ? excludeIds
    : [...new Set([...(excludeIds || []), ...((replacementPlan?.displacementIds || []))])];
  const block = replacementPlan?.blocked
    ? { kind: "protected", block: { entry: (replacementPlan.pinnedEntries || [])[0] || null, reason: "pinned" }, candidate: candidates[0] }
    : getDragPreviewBlockLight(candidates, { excludeIds: previewExcludeIds });
