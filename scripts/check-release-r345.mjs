...
const expected = "2026-07-14-school-year-workspaces-r345";
...
function check(file) {
  ...
  const activeOld = [
    "2026-07-13-system-audit-r343",
    "2026-07-14-word-3split-row-height-r344",
  ...
  if (["index.html", "prework.html", "results.html", "roster.html", "setup.html", "timetable.html", "timetable-print.html", "js/version.js"].includes(rel)) {
    if (!text.includes(expected)) failures.push(`${rel}: expected build version missing`);
  }
  if (rel === "js/config.js" && !text.includes("schoolYears")) failures.push(`${rel}: school-year domain path missing`);
  if (rel === "js/state.js" && !text.includes("ACTIVE_SCHOOL_YEAR")) failures.push(`${rel}: active school-year state routing missing`);
  if (rel === "school-years.html" && !text.includes("copyOnlineWorkspace")) failures.push(`${rel}: safe copy workflow missing`);
}
